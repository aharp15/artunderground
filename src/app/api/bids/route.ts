...
export async function POST(request: NextRequest) {
  ...
  const { data: result, error } = await supabase.rpc('place_bid', { p_auction_id: parsed.data.auction_id, p_amount_gbp: parsed.data.amount_gbp })
...
export async function GET(request: NextRequest) {
  ...
  const { data: bids, error } = await supabase.from('bids').select('id, amount_gbp, placed_at, is_winning').eq('auction_id', auctionId).order('placed_at', { ascending: false }).limit(50)
