...
export function BiddingPanel({ artwork, initialAuction }: Props) {
  ...
  if (!live) return (
    <div className='p-6'>
      <p className='text-2xl font-medium'>
        {artwork.price_gbp != null ? <Price gbp={artwork.price_gbp} /> : 'POA'}
      </p>
    </div>
  )
  ...
  return (
    <div className='p-6 space-y-4'>
      ...
      <div>
        <div className='text-3xl font-medium'><Price gbp={live.current_bid_gbp ?? live.reserve_gbp} /></div>
        <div className='text-sm text-gray-400'>{live.bid_count} bids</div>
      </div>
      {isLive && (
        <div className='flex gap-2'>
          <input type='number' value={bidInput} onChange={e => { setBidInput(e.target.value); setFeedback(null) }}
            placeholder={'Min ' + format(currentBid + 50)}
      ...
      <div>
        ...
        {bids.map((bid, i) => (
          <div key={bid.id} className='flex justify-between py-1.5 text-sm border-b border-gray-50'>
            ...
            <span className='font-medium'><Price gbp={bid.amount_gbp} /></span>
          </div>
        ))}
      </div>
    </div>
